...
#data_recon.Cognition_ceradsc = data_recon.Cognition_ceradsc + 1
...
orig = pd.read_csv('../data_python/Cluster_1.csv', header=None)
names = pd.read_csv('../python_names/Cluster_1_cols.csv')
orig.columns = names['x'].values.tolist()

for f in files[1:]:
    orig_vars = pd.read_csv('../data_python/'+re.sub('.csv','',f)+'.csv', header=None)
    names = pd.read_csv('../python_names/'+re.sub('.csv','',f)+'_cols.csv')
