...
data_all = readRDS("../data/Mayo/data_condensed_complete.rds")
library(ggdendro)
library(ade4)
library(PTXQC)
...
for(i in 1:length(data_all)){
  ...
  if("age" %in% colnames(data_all[[i]])){
    write.table(sv, paste0("../python/Mayo/data_python/","stalone_types.csv"),  sep=',',row.names = F,col.names = T,quote=T, na = "NaN")
  } else{
    write.table(sv,paste0('../python/Mayo/data_python/',dataFrameNames,'_types.csv'),sep=',',row.names = F,col.names = T,quote=T, na = "NaN")
  }
